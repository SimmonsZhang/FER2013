...
def maxpool(x):
    return tf.nn.max_pool2d(x, ksize=[1, 3, 3, 1],
    ...
    with tf.Session() as sess:
        ...
        for step in range(max_train_steps):
            ...
            if step + 1 == max_train_steps:
                saver.save(sess, './', global_step=step + 1)
            if step % 1000 == 0:
...
def valid_model(modelPath, train_data):
    ...
    probs = tf.nn.softmax(y_conv)
    correct_predict = tf.equal(tf.arg_max(probs, 1), tf.math.argmax(y_, 1))
    ...
    with tf.Session() as sess:
        ...
        # tensors = image_to_tensor(validation_images)
        print('', sess.run(probs, feed_dict={x: validation_images, y_: validation_labels})[0])
        print(validation_labels[0])
        val_accuracy = accuracy.eval(feed_dict={x: validation_images, y_: validation_labels})
        ...
        #     pre_emotion = EMOTIONS[result.argmax()]
        #     plt.title(pre_emotion)
